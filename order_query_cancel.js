...
function generateSignedHeaders(method, apiPath, payload, apiKey, secretKey, uid) {
    ...
    return {
        ...commonHeaders, 'sign': signature, 'api_key': apiKey, 'timestamp': timestamp.toString(), 'X-USER-ID': uid,
    };
}
...
function placeOrders(user, INSTRUMENT) {
    ...
    const success = check(response, {
        '下单状态为200': (r) => r.status === 200, '下单响应成功': (r) => {
            try {
...
function queryCurrentOrders(user, INSTRUMENT) {
    ...
    const success = check(response, {
        '查询状态为200': (r) => r.status === 200, '查询响应成功': (r) => {
            try {
                const data = JSON.parse(r.body);
                return data.code === 0;
            } catch (e) {
...
function cancelBatchOrders(user, orderIds, INSTRUMENT) {
    ...
    const success = check(response, {
        '撤单状态为200': (r) => r.status === 200, '撤单响应成功': (r) => {
            try {
                const data = JSON.parse(r.body);
                return data.code === 0;
            } catch (e) {
